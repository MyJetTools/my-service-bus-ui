...
pub fn status_bar_widget(cx: Scope) -> Element {
    ...
    let persist_queue: LazyNodes<'_, '_> = if calculated_values.persist_queue < 5000 {
        rsx! { b { style: "color:green", "{calculated_values.persist_queue}" } }
    } else {
        rsx! { b { style: "color:red", "{calculated_values.persist_queue}" } }
    };

    let connected = if main_state.status_bar.connected {
        rsx! { b { style: "color:green", "Yes" } }
    } else {
        rsx! { b { style: "color:red", "No" } }
    };
    ...
    render! {
        table { id: "status-bar",
            tr {
                ...
                td { div { class: "status-bar-separator" } }
                td { "Persist queue: ", persist_queue }
                td { div { class: "status-bar-separator" } }
